$(document).ready(function() {
	...
	if(stoName != null) {
		$(".login-group").hide();
		$(".user-group").css("display","inline-table");
		$(".loginuser").text("用户ID "+stoName);
		$("#quit").click(function() {
			logout();
		});
	} else {
...
function logout() {
	$.ajax({
		type: "POST",
		url: context + "/service/bd/login/logout",
		success: function(result) {
		},
		error: function() {
		}
	});
    $.removeCookie("username", {path: "/bd"});
    $(".user-group").hide();
    $(".login-group").css("display","inline-block");
...
//调整到注册页面
function regist(){
	window.location.href = context + "/jsp/regist/register.jsp";
}
//初始明细页内容
